...
bell <- "/Users/bell/Downloads/LTREB-life-history"
setwd(bell)
...
for (sp in species_list) {
  sp_data <- subset(posterior_summary_surv, species == sp) #data for each species
  lines(sp_data$relage, sp_data$prob_pos, col = species_colors[sp], lwd = 2) }
legend("bottomright", legend = species_list, col = species_colors,
       lty = 1, lwd = 2, cex = 0.9, bty = "n", title = "Species")
}
dev.off()
...
for (sp in species_list) {
  sp_data <- subset(posterior_summary_fert, species == sp) #data for each species
  lines(sp_data$relage, sp_data$prob_pos, col = species_colors[sp], lwd = 2) }
legend("bottomleft", legend = species_list, col = species_colors,
                                                                                     lty = 1, lwd = 2, cex = 0.9, bty = "n", title = "Species")
}
